...
module.exports= {
  ...
  parking: async(req, res, next)=>{
    ...
    if (user.money<5000){
      throw new ErrorResponse(400, "Tài khoản không đủ tiền gửi xe. Yêu cầu nạp thêm tiền");
    }
    //trừ tiền gửi xe, mỗi lần gửi trừ 5000
    let userUpdate= await userModel.findByIdAndUpdate(user._id, {money: user.money-5000}, {new: true})
    if (!userUpdate){
